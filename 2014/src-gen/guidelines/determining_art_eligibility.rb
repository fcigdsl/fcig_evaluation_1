module DeterminingArtEligibility
	...
	def self.universal_ART_eligibility_in_children(patient)
		actions = []
		valid_condition = true
		...
		valid_condition = PatientService.evaluate_concept_condition('On ART', 'is', 'No', patient) unless !valid_condition
		valid_condition = PatientService.evaluate_concept_condition('age', '>=', '12', patient, 'months') unless !valid_condition
		valid_condition = PatientService.evaluate_concept_condition('age', '<', '60', patient, 'months') unless !valid_condition
		
		
		if valid_condition
			actions = []
		
		actions << ['Flag patient for', 'ART', 'universal ART eligibility in children']
	...
	def self.guideline_recommendations(patient)
		...
		alerts = alerts + universal_ART_eligibility_in_children(patient)
